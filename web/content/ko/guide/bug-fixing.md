...
특정 버그를 진단하고 수정해야 할 때 `/debug`를 사용합니다(또는 자연어로 "버그 수정", "에러 수정", "디버그"라고 말합니다). 이 워크플로우는 증상이 아닌 근본 원인을 수정하는 구조화된 재현 가능한 디버깅 접근법을 제공합니다.
...
사전에 더 많은 컨텍스트를 제공할수록 디버그 워크플로우에서 필요한 질문 왕복이 줄어듭니다.
...
**대응 기대:** 모든 것을 중단. 해결될 때까지 이것이 유일한 태스크.
...
**디버그 접근:** 전체 템플릿을 건너뜁니다. 에러 메시지와 스택 트레이스를 제공합니다. 워크플로우는 즉시 Step 2(재현)에서 시작합니다.
...
**정의:** 기능은 작동하지만 저하된 동작. 사용성에 영향을 주지만 기능성에는 아님.
...
**대응 기대:** 백로그에 추가. 편리할 때 수정하거나 관련 변경과 배치 처리.
...
`/debug` 워크플로우는 이 단계를 엄격한 순서로 실행합니다. 전 과정에서 MCP 코드 분석 도구를 사용합니다 — raw 파일 읽기나 grep이 아닙니다.
...
핵심 원칙: **근본 원인**을 진단합니다, 증상이 아닙니다. `user.id`가 undefined이면, 질문은 "undefined를 어떻게 검사하지?"가 아니라 "이 실행 경로에서 user가 왜 undefined인가?"입니다.
...
**조치:** 먼저 테스트 인프라를 수정한 다음(또는 `/setup`으로 설정) 디버그 워크플로우로 돌아옵니다.
