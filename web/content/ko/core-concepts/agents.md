...
| 카테고리 | 에이전트 | 책임 |
|----------|--------|---------------|
| **아이디어** | oma-brainstorm | 아이디어 탐색, 접근 방식 제안, 설계 문서 작성 |
...
**스택 감지:** 프로젝트 매니페스트(pyproject.toml, package.json, Cargo.toml, go.mod 등)를 읽어 언어와 프레임워크를 결정합니다. 있으면 `stack/` 디렉토리로 폴백하거나, 사용자에게 `/stack-set` 실행을 요청합니다.
...
- 컬러: 보라-파랑 그라디언트 금지, 그라디언트 오브/블롭 금지, 순수 흰색 위의 순수 검정 금지
